...
export class SendEmailHelper {
  ...
  async sendWelcomeEmail(emailTo: User) {
    const email = {
      body: {
        name: `${
        ...
        intro: [
          'Welcome to StarkPay -  Your journey begins here (pun intended 😉)',
          `We are thrilled to have you onboard as a valued member of StarkPay.`,
          `We believe you've made an excellent choice.`,
          `Let's embark on this journey together, simplifying the way you pay for your trips. Thank you for joining us on this ride!`,
        ],
        outro:
  ...
  async sendPinResetEmail(emailTo: User, Token: string) {
    try {
      const email = {
        body: {
          title: 'StarkPay Pin Reset',
          name: `${emailTo.firstName + ' ' + emailTo.lastName}`,
          intro: [
            `Please use the otp to complete your StarkPay pin reset: <h1 style="color:black"><b>${Token}</b></h1>`,
          ],
          outro:
            "Need help, or have questions? Just reply to this email, we'd love to help.",
        },
